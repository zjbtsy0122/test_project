...
def start_process(process_path):
    bat_file_path = r"D:\testclient\start_program.bat"
...
if __name__ == '__main__':
    try:
        ...
        while True:
            ...
            logging.info(f"压力测试正式开始！'{option}'的当前值为: {current_value} (压测运行次数: {loop_count})")
            time.sleep(running_time)  # 等待运行时间
            close_process(process_name)  # 关闭进程
            current_time = time.strftime('%Y年%m月%d日 %H时%M分%S秒', time.localtime())
            logging.info(f"结束时间: {current_time}")
            ...
            if new_value > finish_value:  # 检查是否超过结束值
                logging.info(f"新值{new_value}大于finish值：{finish_value}，停止程序。")
                break
