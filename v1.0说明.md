...
1. **用户输入**：
   - 客户端启动后，提示用户输入指令。目前支持的指令是 `Find`，用于请求所有服务器返回特定的配置信息。
   - `Find_`+`c_cfg.ini`配置文件sections，举例`Find_172.190.117.203:6020`。
...
6. **保存结果**：
   - 将解析后的数据以配置文件的形式`result.ini`保存到同级目录，供用户查看。
...
确保你有 Python 安装在你的系统中(也可以把py文件打成exe文件)，并且你已经配置好所需的文件和目录结构：
...
# 在服务器端
"C:\Program Files\Python310\python.exe" D:\test_project\sever_setting\sever.py 
Server listening on ('127.0.0.1', 21563)
waiting for message...
...
# 在客户端
"C:\Program Files\Python310\python.exe" D:\test_project\sever_setting\client.py 
请输入指令（Find）：Find
Sending request: Find-../tradetrans/ to ('127.0.0.1', 21563)
Received raw data: b'jsdspxa_common-172.192.48.135-6910,jsdspxa_credit-172.190.111.75-9081,jsdspxgoldexch_common-172.192.48.9-16800'
Parsed data parts: ['jsdspxa_common', '172.192.48.135', '6910']
