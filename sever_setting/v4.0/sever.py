...
class Server_pq():  # 定义服务器类
    ...
    def get_data(self, path, section, item, file='TradeTrans_private.ini'):  # 从具体文件中取数据
        conf = configparser.ConfigParser(strict=False)  # 创建配置解析器对象
        ...
        for sec in section:
            if conf.has_section(sec):
                ...
                for j in range(len(item)):
                    value.append(conf.get(sec,item[j]))
    ...
    def select_action(self, data):  # 操作选择方法
        if data[0:5] == 'Find-':  # 如果操作为查找
            ...
            # find_section = data[5:]
            # result = self.get_data('',find_section,self.item)
            # result = self.change_aryy(result)
            return result  # 返回结果
        else:
            print("NO!!!")  # 操作无效
